...
par(mfcol=c(2,2))
# Prepares an empty 2x2 graphics table to be filled from top to bottom then from left to right

with(data, plot(data2, data$Global_active_power, type="l", xlab="", ylab="Global Active Power", cex.lab=.75, cex.axis=.75))
# Plots the global active power as a function of the time, with the correct labels and text size
...
with(data, {
        ...
        legend("topright", legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), col=c("black", "red", "blue"), lty=c(1,1,1), cex=.75, text.width=90000, bty="n")
})
...
with(data, plot(data2, data$Voltage, type="l", xlab="datetime", ylab="Voltage", cex.lab=.75, cex.axis=.75))
# Plots the voltage as a function of the time, with the correct labels and text size

with(data, plot(data2, data$Global_reactive_power, type="l", xlab="datetime", ylab="Global_reactive_power", cex.lab=.75, cex.axis=.75))
# Plots the global reactive power as a function of the time, with the correct labels and text size
